...
def SRTN(num_of_processes, processes):
	print "~~~ Shortest Remaining Time Next scheduling ~~~"
	...
	while total_burst_time != 0:
		...
		if len(arrived_processes) == 0:
			...
			# Find index of arrived process with the least burst time
			index_min_burst_time = arrived_processes[0]
			for arrived in arrived_processes:
				if temp_burst_time[arrived] < temp_burst_time[index_min_burst_time]:
					index_min_burst_time = arrived
				# If both have the same burst time, choose the one arrive earlier
				elif temp_burst_time[arrived] == temp_burst_time[index_min_burst_time]:
					if processes[arrived].arrival_time < processes[index_min_burst_time].arrival_time:
						index_min_burst_time = arrived
		
			# Update burst time
			temp_burst_time[index_min_burst_time] -= 1
			
			# Process finish executing
			# Update total burst time, total turnaround time and total waiting time
			if temp_burst_time[index_min_burst_time] == 0:
				total_burst_time -= processes[index_min_burst_time].burst_time
				finish_time = current_time + 1
				turnaround_time = finish_time - processes[index_min_burst_time].arrival_time
				waiting_time = turnaround_time - processes[index_min_burst_time].burst_time
			...
			if current_time == 0 or pnames[-1] != processes[index_min_burst_time].pid:
				pnames.append(processes[index_min_burst_time].pid)
				ptimes.append(current_time)
